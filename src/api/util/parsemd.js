...
const replaceRegex = (regex, replacement) => (str) => str.replace(regex, replacement);
...
const inlineCodeReplacer = (fullMatch, tagStart, tagContents) => `<code>${tagContents}</code>`;

const imageReplacer = (fullMatch, tagTitle, tagURL) => `<img src="${tagURL}" alt="${tagTitle}" />`;
const linkReplacer = (fullMatch, tagTitle, tagURL) => `<a href="${tagURL}">${tagTitle}</a>`;
const headingReplacer = (fullMatch, tagStart, tagContents) => `\n<h${tagStart.trim().length}>${tagContents}</h${tagStart.trim().length}>`;
const boldItalicsReplacer = (fullMatch, tagStart, tagContents) => `<${(tagStart.trim().length === 1) ? ('em') : ('strong')}>${tagContents}</${(tagStart.trim().length === 1) ? ('em') : ('strong')}>`;
const strikethroughReplacer = (fullMatch, tagStart, tagContents) => `<del>${tagContents}</del>`;
...
const codeBlockFixer = (fullMatch, tagStart, tagContents, lastMatch, tagEnd) => {
  let lines = '';
  tagContents.split('\n')
    .forEach((line) => {
...
const replaceMarkdown = (str) => replaceParagraphs(replaceOrderedLists(replaceUnorderedLists(
...
const parseMarkdown = (str) => fixCodeBlocks(replaceMarkdown(`\n${str}\n`)).trim();
